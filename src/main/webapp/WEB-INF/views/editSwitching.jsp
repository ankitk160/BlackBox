	...
	<main class="main-wrapper">
		<div class="main-content">
			...
			<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
				<div class="breadcrumb-title pe-3">Update Switching Group</div>
				<div class="ps-3">
				...
				<div class="col-lg-12 mx-auto">
					<div class="card">
						<div class="card-body p-4">
							<form id="addSwitchingValidation" action="updateSwitchData"
								method="post">
								<div class="row mb-3">
									...
									<div class="col-sm-9">
										...
										<input type="hidden" name="id" value="${switching.id}">
									</div>
								</div>
								<div class="row mb-3">
									...
									<div class="col-sm-9">
										<select class="form-select" id="operator" name="operator"
											...
											<option selected="selected" value="${offer.operatorid}">${offer.operatorName}</option>
										</select>
									</div>
								</div>
								<div class="row mb-3">
										...
										<select class="form-select" id="campaign1" name="campaign1"
											...
											<c:forEach items="${filteredCampaigns}" var="campaign">
												<option value="${campaign.id}"
													<c:if test="${switching.campid1 == campaign.id}">selected</c:if>>${campaign.id}-
								...
								<div class="row mb-3">
									...
									<div class="col-sm-9">
										<select class="form-select" id="campaign2" name="campaign2"
											...
											<c:forEach items="${filteredCampaigns}" var="campaign">
												<option value="${campaign.id}"
													<c:if test="${switching.campid2 == campaign.id}">selected</c:if>>${campaign.id}-
													${campaign.description}</option>
											</c:forEach>
										</select>
									</div>
								...
								<div class="row mb-3">
									...
									<div class="col-sm-9">
										<select class="form-select" id="campaign3" name="campaign3"
											...
											<c:forEach items="${filteredCampaigns}" var="campaign">
												<option value="${campaign.id}"
													<c:if test="${switching.campid3 == campaign.id}">selected</c:if>>${campaign.id}-
													${campaign.description}</option>
											</c:forEach>
										</select>
									</div>
								...
								<div class="row mb-3">
									...
									<div class="col-sm-9">
										<select class="form-select" id="campaign4" name="campaign4"
											...
											<c:forEach items="${filteredCampaigns}" var="campaign">
												<option value="${campaign.id}"
													<c:if test="${switching.campid4 == campaign.id}">selected</c:if>>${campaign.id}-
													${campaign.description}</option>
											</c:forEach>
										</select>
									</div>
